...
3) 그 다음 점을 보면서
   	- 해당 점과 가장 최근 벡터가 시계 방향으로 배치되어 있다면 감싸는 형태가 되므로 윗 껍질을 구성하는 녀석으로 판단하여 Stack에 넣는다. (구문 상으로는 break)
	- 반시계나 직선 배치라면 Stack을 pop한다.
 	- 다시 해당 점과 가장 최근 두 점의 벡터가 시계 방향으로 배치되어 있는지 확인한다.
4)  해당 점을 Stack에 넣는다.
5) 아래껍질도 윗껍질과 똑같은 원리로 반시계 방향으로 검사한다.
...
- table[i]는 0부터 i번째 문자일때 접두사/접미사가 얼마나 겹치는가 이다.
